...
```
$ git clone https://github.com/aiidateam/aiida-quantumespresso-ph
$ cd aiida-quantumespresso-ph
$ pip install -e .
```
...
Originally discussed in [this issue](https://github.com/aiidateam/aiida-quantumespresso-ph/issues/1127).
...
* `origin`: remote pointing to `aiidateam/aiida-quantumespresso-ph`
* `fork`: remote pointing to personal fork, e.g. `mbercx/aiida-quantumespresso-ph`
...
* Also check the [compatibility matrix in `README.md`](https://github.com/aiidateam/aiida-quantumespresso-ph?tab=readme-ov-file#compatibility-matrix) to see if any changes have to be made.
...
:::{important}
In case you did push with the wrong tag, the CD should fail since we [validate the tag](https://github.com/aiidateam/aiida-quantumespresso-ph/blob/main/.github/workflows/validate_release_tag.py) as part of the workflow.
