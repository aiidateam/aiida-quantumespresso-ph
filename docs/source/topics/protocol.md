...
## Example for the DynamicalMatrixWorkChain
...
DynamicalMatrixWorkChain = WorkflowFactory('quantumespresso.dynamical_matrix')

builder = DynamicalMatrixWorkChain(
  pw_code=load_code('pw@locahost'),
  ph_code=load_code('ph@locahost'),
  structure=load_node(1234), # substitute 1234 with a StructureData node of your interest
  protocol='fast',
)
...
The Brillouin zone is sampled at $q$-points that are defined by a Monkhorst-Pack mesh including the $\Gamma$-point.
The mesh density is defined in terms of the `qpoints_distance` input, which defined the maximum distance between $q$-points in each reciprocal-space direction (i.e., the protocol chooses the smallest $q$-point mesh with at least the density required by the specified `qpoints_distance`).

| Protocol name | `kpoints_distance` [1/A] |
|---------------|:-----------------------:|
| `fast`        | 0.60                    |
| `balanced`    | 0.30                    |
| `stringent`   | 0.20                    |
